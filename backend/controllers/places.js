...
router.post('/:placeId/comments', async (req, res) => {
    ...
    if (!req.currentUser) {
        return res.status(404).json({ message: 'You must be logged in to use comments.' })
    }
...
router.delete('/:placeId/comments/:commentId', async (req, res) => {
    ...
    if (isNaN(placeId)) {
        ...
        if (!comment) {
            ...
            res.status(403).json({ message: `You don't have permission to delete comment "${comment.commentId}"`})
        } else {
